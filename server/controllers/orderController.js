...
const cancelOrderItem = asyncHandler(async (req, res, next) => {
  ...
  if (canceledItem.status === "Cancelled") {
    const error = new Error("Item is already canceled.");
    error.statusCode = 400;
    return next(error);
  }
  ...
  const product = await Product.findById(canceledItem.productId);
  if (product) {
    product.stock += canceledItem.quantity;
    await product.save();
  } else {
    console.warn(
      `Product with ID ${canceledItem.productId} not found. Stock not updated.`,
    );
  }

  // Save changes to order and wallet
  await Promise.all([order.save(), wallet.save()]);

  message = `Order item cancelled successfully. ₹${refundAmount} credited to wallet and stock updated.`;
